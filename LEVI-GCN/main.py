...
def train(enc, dec, optimizer, scheduler, features, labels, adj_label, args):
    device = args['device']
    enc = enc.to(device)
    dec = dec.to(device)
    enc.train()
    ...
    for epoch in range(args['epochs']):
        ...
        rec_loss_adj = F.binary_cross_entropy(adj_hat, adj_label)
        loss = args['ld'] * rec_loss_y + args['alpha'] * kl_loss + args['beta'] * rec_loss_x + args['gamma'] * rec_loss_adj
        # backward
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        train_reca_loss.append(rec_loss_adj.data.cpu())
        # print
        if epoch % 100 == 99:
            ...
            print('loss: {:.03f} '.format(np.mean(train_loss)), 
                ...
                'reca_loss: {:.03f} '.format(np.mean(train_reca_loss)),
...
def main(args):
    ...
    adj_label, adj_norm = preprocess_graph(loadAdj(args)['adj'], args)
    n_instance = adj_label.shape[0]
    n_feature = train_data.n_feature
    n_label = train_data.n_label
    args['dim_z'] = max(args['dim_z'], n_label)
    # create model
    
    enc = VGAE_Encoder(adj=adj_norm, n_in=n_feature+n_label, n_hid=args['n_hidden'], n_out=args['dim_z'])
    dec = VGAE_Decoder(n_in=args['dim_z'], n_hid=args['n_hidden'], n_out=n_feature, n_label=n_label, keep_prob=args['keep_prob'])
    ...
    train(enc, dec, optimizer, scheduler, features, labels, adj_label, args)
    # enhance label
