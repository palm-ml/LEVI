...
class VAE_Encoder(nn.Module):
    def __init__(self,n_in,n_hid,n_out,keep_prob=1.0):
        super(VAE_Encoder,self).__init__()
        self.n_out = n_out
        self.keep_prob = keep_prob
        self.fc1 = nn.Linear(n_in, n_hid)
        self.fc2 = nn.Linear(n_hid, n_hid)
        self.fc_out = nn.Linear(n_hid,n_out*2)
        self._init_weight()
    ...
    def forward(self,inputs,eps=1e-8):
        h0 = self.fc1(inputs)
        h0 = F.dropout(F.elu(h0), 1-self.keep_prob)
        h1 = self.fc2(h0)
        h1 = F.dropout(F.tanh(h1), 1-self.keep_prob)
        out = self.fc_out(h1)
        mean = out[:,:self.n_out]
        std = F.softplus(out[:,self.n_out:]) + eps
        return (mean,std)
